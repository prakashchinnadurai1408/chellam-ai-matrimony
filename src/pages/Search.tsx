...
const getAgeFromDate = (dateOfBirth?: string | null) => {
...
const SearchPage = () => {
  ...
  const runAiMatching = async () => {
    ...
    try {
      ...
      if (!session) {
        toast.error("Log in to run AI matching.");
        return;
      }
      ...
      if (!myProfile) {
        toast.error("Complete your profile before using AI match.");
        return;
      }
      ...
      toast.success(`AI scored ${matches.length} profiles using the Python backend.`);
    } catch (e) {
      console.error("AI matching failed:", e);
      toast.error("AI matching failed. Check that the FastAPI backend is running.");
    } finally {
  ...
  const [incomeRange, setIncomeRange] = useState([0, 100]);
  // Horoscope
  ...
  const activeFilterCount = [
    ...
    verifiedOnly, minMatchScore[0] > 0,
    heightRange[0] !== 54 || heightRange[1] !== 84,
    incomeRange[0] !== 0 || incomeRange[1] !== 100,
    selectedRashi, selectedNakshatra,
  ...
  const filtered = useMemo(() => {
    let results = combinedProfiles.filter((p) => {
      ...
      // Handle Height Parsing (ft'in")
      if (p.height) {
        const hMatch = p.height.match(/(\d+)'(\d+)"/);
        if (hMatch) {
          const totalInches = parseInt(hMatch[1]) * 12 + parseInt(hMatch[2]);
          if (totalInches < heightRange[0] || totalInches > heightRange[1]) return false;
        }
      }

      // Handle Income Parsing (X-Y LPA or X LPA)
      if (p.income) {
        const iMatch = p.income.match(/(\d+)(?:-(\d+))?\s*LPA/);
        if (iMatch) {
          const minI = parseInt(iMatch[1]);
          const maxI = iMatch[2] ? parseInt(iMatch[2]) : minI;
          // Intersection check: if profile range completely outside filter range
          if (maxI < incomeRange[0] || minI > incomeRange[1]) return false;
        }
      }
    ...
    return results;
  }, [combinedProfiles, ageRange, selectedReligion, selectedCastes, selectedEducations, selectedProfessions, selectedStates, selectedMarital, verifiedOnly, minMatchScore, heightRange, incomeRange, selectedRashi, selectedNakshatra, manglikFilter, minGuna, searchQuery, sortBy]);
                ...
                >
                  <div className="bg-card rounded-2xl border border-border/50 shadow-card p-5 sticky top-24">
                    ...
                    <div className="max-h-[calc(100vh-200px)] overflow-y-auto pr-1 space-y-0">
                      ...
                      {/* Height */}
                      <FilterSection title="Height" icon={Ruler}>
                        <Slider
                          defaultValue={[54, 84]}
                          min={54}
                          max={84}
                        ...
                        <div className="flex justify-between text-xs font-medium text-muted-foreground mt-2">
                          <span>{Math.floor(heightRange[0]/12)}'{heightRange[0]%12}"</span>
                          <span>{Math.floor(heightRange[1]/12)}'{heightRange[1]%12}"</span>
                        </div>
                      </FilterSection>

                      {/* Income */}
                      <FilterSection title="Annual Income (LPA)" icon={Star}>
                        <Slider
                          defaultValue={[0, 100]}
                          min={0}
                          max={100}
                        ...
                        />
                        <div className="flex justify-between text-xs font-medium text-muted-foreground mt-2">
                          <span>{incomeRange[0]} LPA</span>
                          <span>{incomeRange[1] === 100 ? "100+ LPA" : `${incomeRange[1]} LPA`}</span>
                        </div>
                      </FilterSection>
                    ...
                    >
                      <Link to={`/profile/${profile.id}`}>
                        <div className="relative aspect-[4/5] overflow-hidden">
                          <img src={profile.image} alt={profile.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
