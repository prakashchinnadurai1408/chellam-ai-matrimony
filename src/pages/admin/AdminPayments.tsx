...
import { QrCode, Upload, Save, Check, X, Eye, Clock, IndianRupee, ShieldAlert, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { hasAccess } from "@/config/rbac";

const AdminPayments = () => {
  const { user } = useAuth();
  const { role, loading: roleLoading } = useAdminRole(user?.id);
  ...
  useEffect(() => {
    if (!roleLoading && hasAccess(["admin"], role)) {
      fetchSettings();
      fetchRequests();
    }
  }, [roleLoading, role]);
    ...
    return (
      <AdminLayout>
        <div className="p-8 text-center space-y-4 max-w-sm mx-auto mt-20">
          ...
          <p className="text-muted-foreground">Only Super Admins can access Payment settings.</p>
        </div>
      </AdminLayout>
    );
  }
